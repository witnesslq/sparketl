...
object ShopInfoJob {
  ...
  def etlUnion(etlConfig: ShopInfoConfig, areaDF: DataFrame, logicDF: DataFrame)(implicit sqlContext: SQLContext) = {
    ...
    val tmpDF = tDF
      ...
      .selectExpr("SRC_SHOP_NO", "getShopName(SHOP_NM,BRAND_NM)  SHOP_NM", "MCHNT_SECOND_PARA", "area_nm",
        ...
        "SHOP_LAT", "getShopTime(BUSI_TIME) BUSI_TIME", "PARK_INF", "BRAND_NO", "SHOP_CONTACT_PHONE",
      ...
      .mapPartitions(it => {
        ...
        it.map(row => {
          ...
          val areaNm = row.getAs[String]("area_nm")
          ...
          val brandNo = row.getAs[Long]("BRAND_NO")
          ...
          (srcShopNo, shopNm, mchntSecondPara, areaNm, provCd, cityCd, countyCd, shopAddr,
            lng, lat, busiTime, parkInf, brandNo, shopContactPhone, mchntSt, shopRate, shopAveConsume, landMarkId)
        })
      })
      .toDF("SRC_SHOP_NO", "SHOP_NM", "MCHNT_SECOND_PARA", "area_nm", "PROV_CD", "CITY_CD", "COUNTY_CD", "SHOP_ADDR",
        "SHOP_LNT", "SHOP_LAT", "BUSI_TIME", "PARK_INF", "BRAND_NO", "SHOP_CONTACT_PHONE", "MCHNT_ST", "SHOP_RATE", "SHOP_AVE_CONSUME", "LANDMARK_ID")
    ...
    val finalDF = tmpDF.as('a)
      ...
      .selectExpr("a.*", "coalesce(d.area_no,'')  area_no", "coalesce(c.industry_no,'') industry_no", "coalesce(c.industry_sub_no,'') industry_sub_no")
...
//      "case when length(trim(split(cast(SHOP_CONTACT_PHONE as string),',')[1]))>20 then '' else trim(split(cast(SHOP_CONTACT_PHONE as string),',')[1]) end SHOP_CONTACT_PHONE", "SHOP_ST",
      ...
      "industry_no", "industry_sub_no", "ROW_CRT_USR", "ROW_CRT_TS", "REC_UPD_USR", "REC_UPD_TS", "LANDMARK_ID"
    )
  ...
  def etlCrawl(etlConfig: ShopInfoConfig, areaDF: DataFrame, brandDF: DataFrame, logicDF: DataFrame)(implicit sqlContext: SQLContext) = {
    ...
    val finalDF = tmpDF.as('a)
      ...
      .selectExpr("a.tradingArea", "a.city", "a.src_shop_no", "a.src_shop_tp", "a.shop_nm",
        ...
        "a.shop_rate", "a.shop_ave_consume*100  shop_ave_consume", "a.landmark_id"
      )
